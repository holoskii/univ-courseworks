...
Date::Date(const Date& Original){ // copy constructor
	Day = Original.Day;
	Year = Original.Year;
...
Date Date::CreateRandomDate(Date begin, Date end){   // returns a date between begin and end
...
Date Date::CreateRandomDate(){
	return Date::CreateRandomDate(Date(1, 1, 2010), Date::Date());
}